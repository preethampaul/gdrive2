        ...
        if user_name == 'default':
            if os.path.exists(INFO_PATH):
                ...
                if check_only:
                    break
                user_name = input(
                    "<default> user_name not yet created; try another user_name : ")
                prob = True
...
def create_info(info, parent_path=DEFAULT_INFO[DEFAULT_INFO['default_parent']][1],
    ...
    if len(info) == 0:
        ...
        if not parent_name in list(info.keys()):
            ...
            info[parent_name] = [info[def_parent][0], parent_path,
                                 *DEFAULT_INFO[DEFAULT_INFO["default_parent"]][2:]]
    ...
    info[parent_name][2] = get_path_ids(
        parent_path, drive, create_missing_folders=False, path_to='folder', default_root=drive_id)[-1]
...
def check_client(client=None):
    ...
    if client == None:
        ...
        while not (client_num > 0 and client_num <= len(clients_list)):
            client_num = input(
                "Enter the sr. no. of client name from list above: ")
            try:
...
def init(args):
    ...
    if '--add-client' in args:
        ...
        client_path = input(
            "Enter absolute path to the new client secrets file: ")
        client = input(
            "Enter new client <name> as in <name>.json with no spaces : ")
    ...
    if len(args) == 0:
        ...
        if parent_name == '-user':
            ...
            if len(args) == 2:
                ...
                if user_name == '-a':
                    ...
                    if prompt == 'y':
                        ...
                        if os.path.exists(INFO_PATH):
                            ...
                            print(
                                "All authentication data deleted. Use `gd init` to re-initialize.")
                    ...
                    if len(invalid_pars) == 0:
                        ...
                        print("user_name : '" + user_name +
                              "' is already removed from system")
                        print(
                            "Delete [gd reset <parent_name> -d] or reset user_names [gd reset <parent_name> -user] for these parents :")
                        print(invalid_pars)
                ...
                print(
                    "Unexpected/invalid no. of arguements passed. Use 'gd reset -h' for help.")
                return
        ...
        if not parent_name in parents_list:
            if parent_name[0] != '-':
                ...
                if parent_name == '-info':
                    if len(args) > 1:
                        print(
                            "Unexpected arguements passed. Use 'gd reset -h' for help.")
                        return
            ...
            if len(args) == 2:
                ...
                if args[1] == '-user':
                    ...
                    if not exists_bool:
                        ...
                        print("+ Delete this parent : 'gd reset " +
                              parent_name + " -d'")
                        print(
                            "+ Reset the user for this parent : 'gd reset " + parent_name + " -user'")
                        return
                    ...
                    create_info(info, parent_path, parent_name=parent_name,
                                drive_name=drive_name, drive_id=drive_id, same_user=True)
                    print("\n" + parent_name +
                          "'s path changed. Use 'gd status' to check.")
                    return

                elif args[1] == '-drive':
                    if not exists_bool:
                        print("The username : '" + user_name +
                              "'is not registered. Add a new parent using 'gd init -add' to register username.")
                        print(
                            "Use 'gd reset' to change username or delete this parent.")
                        return

                    auth_from_cred(gauth, user_name, info[parent_name][5])
                    drives_list = gauth.service.drives().list().execute()[
                        'items']
                    ...
                    if parent_name == prev_def:
                        print("'" + parent_name +
                              "' is already the default parent.")
                    else:
                        with open(INFO_PATH, 'w') as file:
                            json.dump(info, file)
                        print("Default changed from '" + prev_def +
                              "' to '" + parent_name + "'.")
                    ...
                    if parent_name == par_def:
                        print(
                            "'" + parent_name + "'is the default parent : cannot delete a default parent.")
                        print(
                            "Try changing another parent to default with : 'gd reset <parent_name> -default'")
                        return
                    else:
                ...
                while prompt != 'y' and prompt != 'n':
                    prompt = input(
                        "Change user_name for this parent ? [y/n] : ")
...
def ls(args):
    ...
    if len(args) == 0:
        ...
        [user_name, parent_path, parent_id, drive_name,
            drive_id, client] = info[parent_name]
        ...
        if len(args) == 2:
            ...
            if not is_id:
                ...
                if not parent_name in parents_list:
                    print("'" + parent_name +
                          "' : parent name not defined before.")
                    return
                ...
                if drive_path == '/':
                    drive_path = '~'
                # ----------------------------------------------------
                drive_path = parse_drive_path(
                    drive_path, drive, parent_id, default_root=drive_id)

                parent_id = get_path_ids(
                    drive_path, drive, create_missing_folders=False, path_to='folder', default_root=drive_id)[-1]
    ...
    if RETURN_RESULT:
        if get_types:
            file_paths_list, file_ids_list, file_types_list, _ = list_all_contents(
                parent_path, init_folder_id=parent_id, drive=drive, dynamic_show=False, tier='curr', show_ids=show_ids, get_types=get_types, default_root=drive_id)
            return [file_paths_list, file_ids_list, file_types_list]

        else:
            file_paths_list, file_ids_list, _ = list_all_contents(
                parent_path, init_folder_id=parent_id, drive=drive, dynamic_show=False, tier='curr', show_ids=show_ids, get_types=get_types, default_root=drive_id)
            return [file_paths_list, file_ids_list]

    _, _, _ = list_all_contents(parent_path, init_folder_id=parent_id, drive=drive,
                                dynamic_show=True, tier='curr', show_ids=show_ids, default_root=drive_id)
...
def find(args):
    ...
    if is_id:
        file_path, ids_list = get_path_from_id(
            drive, file_id, default_root=drive_id)
...
def cd(args):
    ...
    new_parent_id = get_path_ids(
        drive_path, drive, create_missing_folders=False, path_to='folder', default_root=drive_id)[-1]
    new_parent_path = drive_path

    info[parent_name][:5] = [user_name, new_parent_path,
                             new_parent_id, drive_name, drive_id]
...
def rm(args):
    ...
    delete_id = get_path_ids(
        drive_path, drive, create_missing_folders=False, path_to='all', default_root=drive_id)[-1]
    delete_path = drive_path

    if delete_path == '' or delete_id == drive_id:
        prompt = input(
            "WARNING: The entire drive will be deleted. Continue?[y/n] : ")
    elif delete_id == parent_id:
        prompt = input(
            "All files in current parent will be deleted. Continue?[y/n]: ")

    if prompt == 'y':
        delete(drive, drive_path=delete_path, drive_path_id=delete_id,
               hard_delete=hard_delete, default_root=drive_id)
    else:
...
def mkdir(args):
    ...
    if len(args) == 0:
        ...
        _ = get_path_ids(drive_path, drive, create_missing_folders=True,
                         path_to='folder', default_root=drive_id)[-1]
...
def push(args):
    ...
    if len(args) == 0:
        ...
        for path in stage_list:
            ...
            if os.path.exists(path):
                if os.path.isdir(path):
                    upload(path, parent_path, drive,
                           prompt=prompt, default_root=drive_id)
                else:
        ...
        for par in args:
            ...
            [user_name, parent_path, parent_id,
                drive_name, drive_id, client] = info[par]
            ...
            for path in stage_list:
                ...
                if os.path.exists(path):
                    if os.path.isdir(path):
                        ...
                        upload_file_by_id(
                            path, parent_id, drive, prompt=prompt)
                else:
                    miss_paths.append(path)
...
def pull(args):
    ...
    if len(args) > 0:
        ...
        if not parent_name in parent_list:
            print("Parent : " + parent_name +
                  " not found. Add parent using 'gd init -add' before pulling.")
            return
    else:
        parent_name = info['default_parent']
    ...
    for drive_path, drive_path_id in zip(drive_path_list, drive_path_id_list):
        ...
        download(drive, drive_path=drive_path, drive_path_id=drive_path_id,
                 download_path=save_path, prompt=prompt, default_root=drive_id)
